...
if ( $PSVersionTable.PSVersion -lt '7.2.0' ) {
    Write-Warning 'The sesame-dependencies PowerShell build script requires PowerShell Core 7.2+. Install or upgrade your PowerShell version: https://aka.ms/pscore6'
    exit 0
}
...
function Package-Dependencies {
    ...
    switch ( $PackageName ) {
        ffmpeg {
            Get-ChildItem ./bin/* -Include '*.exe' -Exclude 'ffmpeg.exe','ffprobe.exe' | Remove-Item -Force -Recurse
            Get-ChildItem ./lib -Exclude 'zlibstatic.lib','zlib.lib','cmake' | Remove-Item -Force -Recurse
    ...
    Get-Date -Format "yyyy-MM-dd" | Set-Content -Path share/sesame-deps/VERSION
...
function Build-Main {
    trap {
        Write-Host '---------------------------------------------------------------------------------------------------'
        Write-Host -NoNewLine '[SESAME-DEPENDENCIES] '
